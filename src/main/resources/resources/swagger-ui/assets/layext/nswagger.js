...
;layui.define(function (exports) {
  ...
  function _parse() {
    ...
    function add2method(methodmeta) {
      methodmeta['models'] = {};
      // 处理参数的Model
      methodmeta.hasOwnProperty('parameters') && $.each(
          methodmeta['parameters'], function (i, param) {
            var realtype;
            if (param.in == "body") {
    ...
    function resolveRealType(pmeta) {
      ...
      if (pmeta && pmeta.hasOwnProperty("$ref")) {
        ...
        switch (pmeta["type"]) {
          case "array":
          case "object":
            var _pmeta = pmeta[((pmeta["type"] == "array") ? "items"
                : "additionalProperties")];
            var _tmparr = resolveRealType(_pmeta);
            _type = _tmparr["type"];
            _showType = ((pmeta["type"] == "array") ? "array" : "object") + "("
                + _tmparr["showtype"] + ")";
    ...
    function wrapShowType(type) {
      return "<a href=\"javascript:gotoModel('" + type + "');\">" + type
          + "</a>";
    }
  ...
  g_.jsonmock = (function () {
    ...
    var _ = {
      mock: function (model) {
        if (!g_definitions || !g_definitions[model]
            || !g_definitions[model]["properties"]) {
          return "";
        }
          ...
          switch (val["type"]) {
            case "array":
              ...
              if (_tmparr.hasOwnProperty("$ref") && (_parent != _$ref)) {
                _mock[name].push(
                    genMock(g_definitions[_$ref]["properties"], _$ref));
              } else {
                _mock[name].push(genMockData(_tmparr.type, example));
              }
              break;
            case "object":
              _mock[name] = {};
              ...
              if (_tmpobj.hasOwnProperty("$ref") && (_parent != _$ref)) {
                _mock[name]["key"] = genMock(g_definitions[_$ref]["properties"],
                    _$ref);
              } else {
                _mock[name]["key"] = genMockData(_tmpobj.type, example);
              }
              break;
            default:
              _mock[name] = genMockData(val.type, example);
              break;
          }
        }
      });
